...
def animate(t, ell, slerp_e, ce_interp, frustum, h_f, vec_f, ax):
    ...
    surf_e = ax.plot_surface(xmesh_e, ymesh_e, zmesh_e, color = 'b')
...
#     ell.c = [ce_interp[0](times[0]), ce_interp[1](times[0]), ce_interp[2](times[0])]
#     rotation_e = slerp_e(times[0])
#     theta, phi = rotation_e.as_euler('ZYX')[0], rotation_e.as_euler('ZYX')[2]
#     ell.rotate(-theta, phi)
#     xmesh_e, ymesh_e, zmesh_e = ell.get_rotated_mesh()
#     # surf_e = ax.plot_surface(xmesh_e, ymesh_e, zmesh_e, color = 'b', alpha = .5)

#     frust.h = h_interp(times[0])
#     frust.v = [kv_norm_interp[0](times[0]), kv_norm_interp[1](times[0]), kv_norm_interp[2](times[0])]
#     frust.orient_frustum()
#     surf_f = ax.plot_surface(frust.x, frust.y, frust.z, color = 'r', alpha = .5)
#     surf_e = ax.plot_surface(xmesh_e, ymesh_e, zmesh_e, color = 'b', alpha = 1)
#     ax.set_axis_off()
#     plt.savefig("model.svg", bbox_inches = 'tight', transparent = True)
